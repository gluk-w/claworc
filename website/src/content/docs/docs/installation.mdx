...
<Aside type="note">
  For most users, the [installer script](/docs/quickstart) is the recommended approach — it handles environment detection and configuration automatically. Use this guide if you need full control over the deployment.
</Aside>
...
Additional Claworc settings (e.g., `CLAWORC_AUTH_DISABLED`, `CLAWORC_RP_ID`) can be passed as environment variables inside the `docker-compose.yml` `environment:` block. See [Environment variables](/docs/environment-variables) for the full list.

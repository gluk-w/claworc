...
See [Configuration](/docs/models/configuration) for the full reference, including per-instance override options and examples.
...
See [Backups](/docs/backups) for the full guide.
...
See [Shared folders](/docs/shared-folders) for setup and limitations.
...
You can use global providers configured in **Settings**, or click **Add provider** to create a provider that belongs only to this instance. Instance-specific providers are useful when you need a dedicated API key or a provider that other instances should not access. See [Configuration](/docs/models/configuration) for details on both options.

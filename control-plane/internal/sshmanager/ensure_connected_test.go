...
func TestEnsureConnected_FreshConnection(t *testing.T) {
	...
	client, err := mgr.EnsureConnected(context.Background(), "test-instance", orch)
	if err != nil {
...
func TestEnsureConnected_CachedConnectionReuse(t *testing.T) {
	...
	client1, err := mgr.EnsureConnected(context.Background(), "test-instance", orch)
	if err != nil {
		t.Fatalf("first EnsureConnected() error: %v", err)
	}

	// Second call should reuse the cached connection
	client2, err := mgr.EnsureConnected(context.Background(), "test-instance", orch)
	if err != nil {
...
func TestEnsureConnected_UploadFailure(t *testing.T) {
	...
	_, err := mgr.EnsureConnected(context.Background(), "test-instance", orch)
	if err == nil {
		t.Fatal("EnsureConnected() expected error when upload fails")
	}

	// Should not have a cached connection
	if _, ok := mgr.GetConnection("test-instance"); ok {
...
func TestEnsureConnected_ReconnectsAfterDeadConnection(t *testing.T) {
	...
	pubKeyBytes, privKeyPEM, _ := sshkeys.GenerateKeyPair()
	_ = pubKeyBytes
	signer, _ := sshkeys.ParsePrivateKey(privKeyPEM)

...
|-------|------------------|---------|
| [`glukw/claworc-agent`](https://hub.docker.com/r/glukw/claworc-agent) | Latest published OpenClaw release | Runs OpenClaw, the terminal, and SSH |
| [`glukw/claworc-stable`](https://hub.docker.com/r/glukw/claworc-stable) | Latest version flagged stable by [isitstable.com/openclaw](https://isitstable.com/openclaw) | Same as above, but pinned to a community-vetted OpenClaw release |
...
<Tip>
  **Which one should I use?** Pick `glukw/claworc-stable` if you want the safest path — it lags `glukw/claworc-agent` by however long the community needs to vet a new OpenClaw release on [isitstable.com](https://isitstable.com/openclaw), so you skip the days when an upstream release introduces a regression. Use `glukw/claworc-agent` if you want new OpenClaw features the moment they ship.
</Tip>

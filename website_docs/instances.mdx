...
Each instance also has its own `clawdbot.json` for agent behavior (model selection, tools, integrations).

See [Configuration](/configuration) for the full reference, including per-instance override options and examples.
...
Before you can enable models on an instance, configure at least one provider in **Settings**. See [LLM gateway](/llm-gateway) for how to add providers and manage API keys.

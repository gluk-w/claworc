...
> Part of the [Internal Proxy](./internal-proxy.md). The LLM proxy is the
> catch-all (`/`) route of the shared `internal/internalproxy` server; this doc
> covers the LLM-specific virtual-key scheme. For the Composio Connections broker
> served alongside it, see [connections.md](./connections.md).
...
The instance sends its virtual key to the LLM proxy running on the control plane. The
LLM proxy validates the token, resolves the real provider API key, and proxies the request. Real
...
- The internal proxy endpoint (`127.0.0.1:<port>`) is only reachable from inside the container via an
...
SSH agent-listener tunnel (port <port> on container)
     ...
     │
     ▼
Control plane — LLM proxy (127.0.0.1:<port>)
 1. Validate virtual key →  look up LLMProxyKey in SQLite
...
The internal proxy port defaults to `40001` and is configurable via the `CLAWORC_INTERNAL_PROXY_PORT`
environment variable (the legacy `CLAWORC_LLM_GATEWAY_PORT` is still honored as a fallback).

### SSH agent-listener tunnel

The internal proxy listens on `127.0.0.1:<port>` on the control plane — it is not publicly exposed.
...
Generated by `generateVirtualKey()` using `crypto/rand`. Each key is unique, stored in the
`llm_proxy_keys` table, and maps exactly one instance to one provider.

Source: `control-plane/internal/internalproxy/keys.go`.


## How Key Resolution Works

When the LLM proxy receives a request, `authAndResolve` looks up the `claworc-vk-*` token in
`llm_proxy_keys`, loads the associated `LLMProvider`, then calls `resolveRealAPIKey`:
...
- The LLM proxy accepts the `claworc-vk-*` virtual key in any of these locations (tried in order):
  `Authorization: Bearer`, `x-api-key`, `x-goog-api-key`, `?key=` query param.
- The LLM proxy always strips all incoming auth headers before forwarding, then sets the correct
...
- **`openai-completions` / `openai-responses`**: the LLM proxy automatically de-duplicates `/v1`
  if `base_url` already ends with `/v1`.


## Testing with Virtual Keys

This walkthrough shows how to exercise the LLM proxy without a real LLM provider.
...
sqlite3 /app/data/claworc.db \
  "SELECT virtual_key FROM llm_proxy_keys WHERE instance_id=1 AND provider_id=1;"
# → claworc-vk-<token>
```

### Step 5: Call the LLM proxy directly (from the control plane host)
...
The LLM proxy proxies to `https://httpbin.org/anything/v1/chat/completions`, which echoes back
the request — you should see your payload and the `Authorization: Bearer dummy-key` header that
the LLM proxy substituted.
...
| File | Description |
|------|-------------|
| `control-plane/internal/internalproxy/gateway.go` | HTTP proxy, auth, key resolution, request logging |
| `control-plane/internal/internalproxy/keys.go` | Virtual key generation, `EnsureKeysForInstance`, `GetInstanceVirtualKeys` |

...
def metadata(m):
    """Descriptive/pricing columns derived from one OpenRouter catalog entry.

    Split out of to_row() so scripts/openai_to_csv.py can borrow it: OpenAI's
    own /v1/models returns nothing but ids, so it enriches from here.
    """
...
def to_row(m, header):
    row = {**CONST, **metadata(m), "model_id": m["id"], "tag": ""}
